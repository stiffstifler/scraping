...
# открываем выгруженный файл html
with open("blank/index.html") as file:
    # сохраняем данный из файла в переменную src
    src = file.read()

# создаем объект класса BeautifulSoup(html код, парсер html кода)
soup = BeautifulSoup(src, "lxml")

# получаем весь тег .title, .text возвращает только текст
# title = soup.title
# print(title)
# print(title.text)
# print(title.string)
...
# .find - находит и берет данные из 1го найденного результата
# page_h1 = soup.find("h1")
# print(page_h1)
#
# .find_all находит и берет все найденные результаты
# page_all_h1 = soup.find_all("h1")
# print(page_all_h1)

# результат .find_all сохраняется в виде листа который можно итерировать
# for item in page_all_h1:
#     print(item.text)

# .find("div", class_="user__info")
# 1й аргумент - тип объекта, 2й аргумент имя класса
# т.к. class в пайтон это ключевое зарезервированное слово - добвавляем _ в конце
# user_name = soup.find("div", class_="user__name")
# .strip() - обрезает пробел у str
# print(user_name.text.strip())

# можно передавать параметры в виде словаря, если нужно соблюдать 2+ параметра
# user_name = soup.find(class_="user__name").find("span").text
# print(user_name)

# находим все элементы span внутри объекта user__info
# user_name = soup.find("div", {"class": "user__name", "id": "aaa"}).find("span").text
# возвращаем список, поэтому можем выбирать элементы
# print(user_name[0])
